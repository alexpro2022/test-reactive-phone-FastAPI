...
через определенный интервал времени (задается пользователем) httpx-клиент забирает с биржи текущую цену BTC и ETH и логирует тикер валюты, текущую цену и время в UNIX (можно посмотреть по ссылке http://127.0.0.1:5555/tasks). Клиент работает в фоновой периодической celery-задаче.
